...
public class ListFragment extends BaseFragment implements ActivityResultListener,Toolbar.OnMenuItemClickListener,FragmentOnKeyListener,SearchView.OnQueryTextListener{

    @BindView(R.id.tablayout) TabLayout tabLayout;
    @BindView(R.id.toolbar) Toolbar toolbar;
    @BindView(R.id.viewPager) LazyViewPager viewPager;
    ...
    void onScroll(int position){
        if (fragments[position] instanceof FragmentOnKeyListener){
            if (((SplashActivity)getBaseActivity())!=null)
                ((SplashActivity)getBaseActivity()).setFragmentOnKeyListener((FragmentOnKeyListener)fragments[position]);
        }else{
            if (((SplashActivity)getBaseActivity())!=null)
                ((SplashActivity)getBaseActivity()).setFragmentOnKeyListener(null);
        }
    ...
    * */
    ...
    protected void onFragmentInit() {
        fragments = new BaseFragment[]{
                ...
                ,((JCFragment) Fragment.instantiate(getActivity(),JCFragment.class.getName()))
                ,((BBS) Fragment.instantiate(getActivity(),BBS.class.getName())).setBaseActivity(getBaseActivity())
                ,((NetAutoFragment) Fragment.instantiate(getActivity(),NetAutoFragment.class.getName())).setBaseActivity(getBaseActivity())
                };
        viewPager.setAdapter(new LazyFragmentPagerAdapter(Objects.requireNonNull(getActivity()).getSupportFragmentManager()) {
            @Override
        ...
        Objects.requireNonNull(tabLayout.getTabAt(3)).setText("教程");
        Objects.requireNonNull(tabLayout.getTabAt(4)).setText("社区");
        Objects.requireNonNull(tabLayout.getTabAt(5)).setText("示例");
        DrawerLayout drawerLayout =((SplashActivity) Objects.requireNonNull(getActivity())).getDrawerLayout();
        ...
        ((SplashActivity)getActivity()).addActivityResultListener(this);
    }
                    ...
                    if (action!=null){
                        AlertDialogSingle.getAlertDialog(getActivity(),"导入脚本","是否导入:"+title+"\n动作个数:"+action.getCount()+"\n重复次数:"+action.getRepeat()+"\n\n温馨提示:如需做到所有手机均可使用，有关于坐标的命令请使用屏幕变量使用数学表达式表达.\n","导入", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                action.setPath(((ActionListFragment)fragments[0]).getCurrentDir().getAbsolutePath()+"/"+title+".ycf");
                                action.save();
                                EventBus.getDefault().post(RefreshAction.INTANSCE);
                            }
                        },"取消",null).show();
