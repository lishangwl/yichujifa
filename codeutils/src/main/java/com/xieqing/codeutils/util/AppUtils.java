...
public final class AppUtils {
    ...
    public static String getTopPackageName() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            UsageStatsManager usm = (UsageStatsManager) Utils.getApp().getSystemService(Context.USAGE_STATS_SERVICE);
            ...
            ActivityManager am = (ActivityManager) Utils.getApp().getSystemService(ACTIVITY_SERVICE);
    ...
    public static void launchApp(final String packageName) {
        if (isSpace(packageName)) return;
        Intent intent = getLaunchAppIntent(packageName, true);
        if (intent == null){
            return;
        }
        try {
    ...
    public static List<AppInfo> getAppsInfo() {
        try {
            ...
            ToastUtils.showLong("出错啦~~ 加载失败："+error.getMessage());
        }
    ...
    public static void resumeApp() {
        try {
            ...
            for (ActivityManager.RunningTaskInfo taskInfo : taskInfoList) {
                ...
                if (taskInfo.topActivity.getPackageName().equals(Utils.getApp().getPackageName())) {
    ...
    public interface ThreadGetClipTextListener{
        void getClipText(String clip);
    }
    public static void getClipText(final ThreadGetClipTextListener listener){
        ThreadUtils.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                listener.getClipText(getClipText());
            }
        });
    }
    public static String getClipText(){
        ClipboardManager clipboardManager = ((ClipboardManager) Utils.getApp().getSystemService(CLIPBOARD_SERVICE));
    ...
    private static Intent getInstallAppIntent(final File file, final boolean isNewTask) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
            ...
            String authority = Utils.getApp().getPackageName();
    ...
    private static String getForegroundProcessName() {
        ActivityManager am =
                (ActivityManager) Utils.getApp().getSystemService(ACTIVITY_SERVICE);
