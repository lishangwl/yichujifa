...
public final class EncryptUtils {
    ...
    public static String EncodeRC4(String source, String key){
        ...
        return keyc + Base64.encode(RC4(("0000000000" + CutString(MD52(source + keyb), 0, 16) + source).getBytes(), keya + MD52(keya + keyc)),0);
    }

    public static String DecodeRC4(String source, String key) {
        if (source == null || key == null) {
            try {
                return "";
            } catch (Exception e) {
        ...
        result = new String(RC4(Base64.decode(CutString(source + "=", 4),0), cryptkey));
        if (CutString(result, 10, 16).equals(CutString(MD52(CutString(result, 26) + keyb), 0, 16))) {
            return CutString(result, 26);
        }
        result = new String(RC4(Base64.decode(CutString(source + "==", 4),0), cryptkey));
